...
Util.conf_defaults(conf, that, defaults, [
    ['target',             'wo', 'dom', null, 'vnc2cast display rendering Canvas object'],
    ...
    // Ultravnc2cast repeater ID to connect to
    ...
    ['onPasswordRequired', 'rw', 'func', function() { },
        'onPasswordRequired(corba): vnc2cast password is required '],
...
function constructor() {
    ...
    ws.on('open', function() {
        if (corba_state === "connect") {
            updateState('ProtocolVersion', "Starting vnc2cast handshake");
        } else {
    ...
    ws.on('close', function(e) {
        ...
        if (corba_state === 'disconnect') {
            updateState('disconnected', 'vnc2cast disconnected' + msg);
    ...
    if (Websock_native) {
        ...
        updateState('loaded', 'vnc2cast ready: native WebSockets, ' + rmode);
    } else {
        ...
        if ((! Util.Flash) ||
            ...
            updateState('loaded', 'vnc2cast ready: WebSockets emulation, ' + rmode);
        }
    }
    ...
    if (state in {'disconnected':1, 'loaded':1, 'connect':1,
        ...
        if (display && display.get_context()) {
            ...
            if ((Util.get_logging() !== 'debug') ||
                (state === 'loaded')) {
                // Show vnc2cast logo on load and when disconnected if
                // debug is off
                display.clear();
            }
        }
...
// CORBA/vnc2cast initialisation message handler
init_msg = function() {
    ...
    case 'ProtocolVersion' :
        ...
        is_repeater = 0;
        switch (sversion) {
            case "000.000": is_repeater = 1; break; // Ultravnc2cast repeater
            case "003.003": corba_version = 3.3; break;
            case "003.006": corba_version = 3.3; break;  // Ultravnc2cast
            ...
            case "004.001": corba_version = 3.8; break;  // Realvnc2cast 4.6
            default:
    ...
    case 'Authentication' :
        ...
        switch (corba_auth_scheme) {
            ...
            case 1:  // no authentication
                ...
                break;
            case 2:  // vnc2cast authentication
    ...
    case 'SecurityResult' :
        if (ws.rQwait("Vnc2cast auth response ", 4)) { return false; }
        switch (ws.rQshift32()) {
            case 0:  // OK
                // Fall through to ClientInitialisation
                break;
...
/* Normal CORBA/vnc2cast server message handler */
normal_msg = function() {
