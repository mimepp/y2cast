...
Util.conf_defaults(conf, that, defaults, [
    ['target',             'wo', 'dom', null, 'mirror4xcat display rendering Canvas object'],
    ...
    // Ultramirror4xcat repeater ID to connect to
    ...
    ['onPasswordRequired', 'rw', 'func', function() { },
        'onPasswordRequired(corba): mirror4xcat password is required '],
...
function constructor() {
    ...
    ws.on('open', function() {
        if (corba_state === "connect") {
            updateState('ProtocolVersion', "Starting mirror4xcat handshake");
        } else {
    ...
    ws.on('close', function(e) {
        ...
        if (corba_state === 'disconnect') {
            updateState('disconnected', 'mirror4xcat disconnected' + msg);
    ...
    if (Websock_native) {
        ...
        updateState('loaded', 'mirror4xcat ready: native WebSockets, ' + rmode);
    } else {
        ...
        if ((! Util.Flash) ||
            ...
            updateState('loaded', 'mirror4xcat ready: WebSockets emulation, ' + rmode);
        }
    }
    ...
    if (state in {'disconnected':1, 'loaded':1, 'connect':1,
        ...
        if (display && display.get_context()) {
            ...
            if ((Util.get_logging() !== 'debug') ||
                (state === 'loaded')) {
                // Show mirror4xcat logo on load and when disconnected if
                // debug is off
                display.clear();
            }
        }
...
// CORBA/mirror4xcat initialisation message handler
init_msg = function() {
    ...
    case 'ProtocolVersion' :
        ...
        is_repeater = 0;
        switch (sversion) {
            case "000.000": is_repeater = 1; break; // Ultramirror4xcat repeater
            case "003.003": corba_version = 3.3; break;
            case "003.006": corba_version = 3.3; break;  // Ultramirror4xcat
            ...
            case "004.001": corba_version = 3.8; break;  // Realmirror4xcat 4.6
            default:
    ...
    case 'Authentication' :
        ...
        switch (corba_auth_scheme) {
            ...
            case 1:  // no authentication
                ...
                break;
            case 2:  // mirror4xcat authentication
    ...
    case 'SecurityResult' :
        if (ws.rQwait("Mirror4xcat auth response ", 4)) { return false; }
        switch (ws.rQshift32()) {
            case 0:  // OK
                // Fall through to ClientInitialisation
                break;
...
/* Normal CORBA/mirror4xcat server message handler */
normal_msg = function() {
